...
public class SchoolCircleModel implements ISchoolCircleModel {
    ...
    @Override
    public Observable<List<PostListBean>> getAnoListFromNet(int page_index) {
        return mCirclesApi.getCircles(page_index)
...
//                                if (thumbUpsBean.getUid() == mIUserModel.getUserBaseBeanNormal().getId()) {
    ...
    @Override
    public Observable<List<PostListBean>> loadAnoListFromNet(int page_index) {
        lowID = Integer.MAX_VALUE;
        return getAnoListFromNet(page_index);
    }
